...
import { ArrowLeft, Home, Calculator, HelpCircle, Check, Copy } from 'lucide-react';

const RentCalculator = () => {
  const [totalAmount, setTotalAmount] = useState('');
  const [participants, setParticipants] = useState(3);
  const [isWeighted, setIsWeighted] = useState(true);
  const [weights, setWeights] = useState(['120', '100', '80']);
  ...
  const handleCalculate = async (e) => {
    ...
    if (!totalAmount || parseFloat(totalAmount) <= 0) {
      setError('Please enter a valid total rent amount.');
      return;
    }
    ...
    if (isWeighted) {
      const roomWeights = weights.map((w) => parseFloat(w) || 1);
      if (roomWeights.some((w) => w <= 0)) {
        setError('All weights must be positive numbers.');
        setLoading(false);
        return;
      }
      payload.room_weights = roomWeights;
    }

    try {
      const res = await api.post('/api/calculators/rent/', payload);
      setResults(res.data);
    } catch (err) {
  ...
  const copyToClipboard = () => {
    ...
    const splitText = results.splits
      ...
      .join('\n');
    const fullText = `Rent Split Breakdown (Total: ₹${parseFloat(totalAmount).toFixed(2)})\n${splitText}\nAverage Share: ₹${parseFloat(
      results.per_person
    ).toFixed(2)}`;
    ...
    <div className="min-h-screen bg-gray-50/50 flex flex-col">
      ...
      <main className="flex-1 max-w-4xl w-full mx-auto px-4 py-8">
        ...
        <div className="flex items-center gap-3 mb-8">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Rent Split Calculator</h1>
            <p className="text-sm text-gray-500">Calculate custom rent splits fairly based on room sizes/features.</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div className="bg-white border border-gray-100 rounded-2xl p-6 shadow-sm h-fit">
            ...
            <form onSubmit={handleCalculate} className="space-y-5">
              {/* Total Rent */}
              <div>
                <label htmlFor="total-rent" className="block text-sm font-semibold text-gray-700 mb-1.5">
                  Total Monthly Rent (₹)
                </label>
                <input
                  id="total-rent"
              ...
              <div className="flex items-center gap-2 py-1">
                ...
                <label htmlFor="use-weights" className="text-sm font-semibold text-gray-700 cursor-pointer select-none">
                  Split using custom weights (room size / balcony / bath)
                </label>
              </div>
              ...
              {isWeighted && (
                <div className="space-y-3 p-4 bg-gray-50 rounded-2xl border border-gray-100">
                  <div className="flex justify-between items-center mb-1">
                    <span className="text-xs font-bold text-gray-400 uppercase tracking-wider">Roommate Weights</span>
                    <div className="relative group">
                      ...
                      <div className="absolute right-0 bottom-full mb-2 hidden group-hover:block w-48 bg-gray-900 text-white text-[10px] p-2.5 rounded-lg shadow-xl z-50 leading-relaxed">
                        Input square footage of each room (e.g. 120, 100, 80) or score scores from 1-10. Splits are calculated proportionally.
                      </div>
                    </div>
                  </div>
              ...
              >
                {loading ? 'Computing...' : 'Calculate Rent Share'}
              </button>
            </form>
          </div>
            ...
            {results ? (
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-6">
                    <div className="bg-gray-50/70 p-4 border border-gray-100 rounded-xl">
                      <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider block mb-0.5">Total Rent</span>
                    ...
                    {results.splits.map((splitVal, idx) => (
                      <div key={idx} className="flex justify-between items-center p-3.5 bg-gray-50/30 border border-gray-100 rounded-xl hover:bg-gray-50/75 transition">
                        <div>
                          ...
                          {isWeighted && (
                            <span className="text-[10px] text-gray-400 block mt-0.5">Weight score: {weights[idx]}</span>
                          )}
                        </div>
              ...
              <div className="border-2 border-dashed border-gray-200 rounded-2xl p-12 text-center flex-1 flex flex-col justify-center items-center bg-white">
                ...
                <p className="text-sm text-gray-400 max-w-[280px]">
                  Fill in the rent details and weights on the left to see the breakdown.
                </p>
              </div>
            )}
          </div>
