...
import { ArrowLeft, Armchair, Calculator, HelpCircle, Check, Copy } from 'lucide-react';

const FurnitureCalculator = () => {
  const [totalAmount, setTotalAmount] = useState('');
  const [participants, setParticipants] = useState(3);
  const [isWeighted, setIsWeighted] = useState(true);
  const [weights, setWeights] = useState(['4', '2', '2']); // Default weights: e.g. owner gets 4 shares, non-owners get 2
  ...
  const handleCalculate = async (e) => {
    ...
    if (!totalAmount || parseFloat(totalAmount) <= 0) {
      setError('Please enter a valid furniture cost.');
      return;
    }
    ...
    if (isWeighted) {
      ...
      if (parsedWeights.some((w) => w <= 0)) {
        setError('All furniture split weights must be positive numbers.');
        setLoading(false);
        return;
      }
    ...
    try {
      const res = await api.post('/api/calculators/furniture/', payload);
      setResults(res.data);
    } catch (err) {
  ...
  const copyToClipboard = () => {
    ...
    const splitText = results.splits
      ...
      .join('\n');
    const fullText = `Furniture Split Breakdown (Total Cost: ₹${parseFloat(totalAmount).toFixed(2)})\n${splitText}\nAverage Share: ₹${parseFloat(
      results.per_person
    ).toFixed(2)}`;
    ...
    <div className="min-h-screen bg-gray-50/50 flex flex-col">
      ...
      <main className="flex-1 max-w-4xl w-full mx-auto px-4 py-8">
        ...
        <div className="flex items-center gap-3 mb-8">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Furniture Split Calculator</h1>
            <p className="text-sm text-gray-500">Calculate custom splits for shared furniture depending on usage or eventual ownership weights.</p>
          </div>
        </div>
            ...
            <form onSubmit={handleCalculate} className="space-y-5">
              ...
              <div className="flex items-center gap-2 py-1">
                ...
                <label htmlFor="use-weights" className="text-sm font-semibold text-gray-700 cursor-pointer select-none">
                  Split using custom ownership/usage weights
                </label>
              </div>
                  ...
                  <div className="flex justify-between items-center mb-1">
                    ...
                    <div className="relative group">
                      ...
                      <div className="absolute right-0 bottom-full mb-2 hidden group-hover:block w-48 bg-gray-900 text-white text-[10px] p-2.5 rounded-lg shadow-xl z-50 leading-relaxed">
                        Specify weights depending on who gets to keep the furniture or how much they use it (e.g. Owner = 4, Users = 2 each).
                      </div>
                    </div>
                  </div>
              ...
              >
                {loading ? 'Computing...' : 'Calculate Furniture Share'}
              </button>
            </form>
          </div>
            ...
            {results ? (
              ...
              <div className="border-2 border-dashed border-gray-200 rounded-2xl p-12 text-center flex-1 flex flex-col justify-center items-center bg-white">
                ...
                <p className="text-sm text-gray-400 max-w-[280px]">
                  Fill in the furniture cost and split weights on the left to see the breakdown.
                </p>
              </div>
            )}
          </div>
